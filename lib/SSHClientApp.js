...
class SSHClientApp {
    ...
    run(argv) {
        return Helper.PromiseTry(() => {
            this.connectInfo = Helper.parseConnectInfo(argv);
            //console.log('connectInfo:', this.connectInfo);
            this.forwardInfo = Helper.parseForwardInfo(argv);
            //console.log('forwardInfo:', this.forwardInfo);
    ...
    getFile(fileName) {
        return this.getCurrentDir().then(dirInfo => {
            let fullFileName = fileName[0] == '/' ? fileName : path.join(dirInfo.cwd, fileName);
